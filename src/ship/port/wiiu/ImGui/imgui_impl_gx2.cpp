...
void    ImGui_ImplGX2_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (bd->lastDrawTimestamp > GX2GetRetiredTimeStamp())
        GX2WaitTimeStamp(bd->lastDrawTimestamp);
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                GX2DrawIndexedImmediateEx(GX2_PRIMITIVE_MODE_TRIANGLES, pcmd->ElemCount,
    ...
    GX2Flush();
    bd->lastDrawTimestamp = GX2GetLastSubmittedTimeStamp();
}
